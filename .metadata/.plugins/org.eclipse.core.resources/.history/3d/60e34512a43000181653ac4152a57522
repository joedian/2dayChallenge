...
public class StatisticService {
	 ...
	 * time was already validated on controller
	 * @param amount
	 * @return
	 */
	...
	public void updateStatistics() {
		...
		synchronized (transactionQueue) {
			...
			//filter queue by collection only transactions with the range of the last 60 seconds
			//range now to now - 60000
			last60SecondsTransactions = transactionQueue.stream()
					.filter( t -> (t.getTimestamp() >= (Instant.now().toEpochMilli() - 60000)))
